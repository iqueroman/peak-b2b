...
| `suppression_override` | The right end state for coordinated multi-threading into an open-opportunity account. Cut on principle rather than for room: until there is a named owner authorised to relax a suppression, it is a table with no writer — and an override with no accountable author is worse than the suppression it relaxes. `suppression_list` is the same shape and is where it lands | **~900 people** |
